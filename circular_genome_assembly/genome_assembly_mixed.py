...
def reconstruct_genome_scs(reads: List[str], has_errors: bool = False) -> str:
    if not reads:
        return ""
    ...
    if read_len < 10:
        ...
        the_k = int(0.8 * len(next(iter(reads))))
...
def align_best_start_le1(read: str, contig: str) -> Tuple[Optional[int], int]:
...
def contig_has_mismatches(reads: List[str], contig: str) -> bool:
    c = contig.strip().upper()
    for read in reads:
        r = read.strip().upper()
        if not r or not c:
