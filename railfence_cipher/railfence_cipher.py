#! /usr/bin/env python
"""
NB We can generate an integer period starting at 0 with a max at n this way

a_i = abs ( (i + n) % (2*n) - n)

number before rail ends if x and y are steps > 0.

"""


def alternating_counts(N: int, x: int, y: int, start: int = 0) -> int:
    if N <= start:
        return 0
    N0 = N - start
    if x > 0 and y > 0:
        P = x + y
        even = (N0 - 1) // P + 1
        odd = ((N0 - x - 1) // P + 1) if N0 > x else 0
        return even + max(0, odd)
    if x == 0 and y > 0:
        return (N0 - 1) // y + 1
    if y == 0 and x > 0:
        return (N0 - 1) // x + 1
...
class RailPost:
    ...
    def encode(self, text):
        dict_fences = {f: [] for f in range(self.n)}

        for i, c in enumerate(text):
            pos = abs((i + self.p) % (2 * (self.p)) - self.p)
            dict_fences[pos].append(c)

        encoded = []
        for i in range(self.n):
            encoded += dict_fences[i]
        return "".join(encoded)
    ...
    def decode(self, text):
        ...
        for row in range(self.n):
            inc_a, inc_b = 2 * (self.n - 1) - 2 * row, 2 * row
            el = alternating_counts(len(text), x=inc_a, y=inc_b, start=row)
            increment = [v for v in (inc_a, inc_b) if v > 0]
            l_pos = row
            for inc, _ in zip(((el + 1) // len(increment)) * increment, range(el)):
                decoded[l_pos] = text[ad]
                ad += 1
                l_pos += inc
