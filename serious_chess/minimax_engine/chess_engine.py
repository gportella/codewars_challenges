...
class Board:
    ...
    def repetition_count(self, key: Optional[U64] = None) -> int:
        ...
        return self.history.count(target)
    ...
    def make_move(self, from_sq: int, to_sq: int) -> Undo:
        ...
        if prev_en_passant != -1:
            key ^= ZOBRIST_KEYS[("en_passant", prev_en_passant % 8)]
        if self.en_passant != -1:
            key ^= ZOBRIST_KEYS[("en_passant", self.en_passant % 8)]

        if prev_castling != self.castling_rights:
            key ^= ZOBRIST_KEYS[("castling", prev_castling.value)]
            key ^= ZOBRIST_KEYS[("castling", self.castling_rights.value)]

        key ^= ZOBRIST_KEYS["side"]

        self.position_key = to_u64(key)
        self.history.append(self.position_key)
...
if __name__ == "__main__":
    ...
    if sys.argv and len(sys.argv) == 2:
        fen = parse_fen(sys.argv[1])
        print(f"Manual parsing of FEN: {sys.argv[1]}")
    if sys.argv and len(sys.argv) == 3:
        fen = parse_fen(sys.argv[1])
        depth = int(sys.argv[2])
        print(f"Manual parsing of FEN: {sys.argv[1]} and depth: {depth}")
    ...
    while True:
        ...
        if ply % 2 == 1:
            game_moves = ply // 2 + 1
        print(
            f"Ply {ply} (Move {game_moves}) - {current_side.name.capitalize()}'s turn"
        )
